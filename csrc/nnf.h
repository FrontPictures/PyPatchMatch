...
public:
    ...
    NearestNeighborField(const MaskedImage &source, const MaskedImage &target, const PatchDistanceMetric *metric, int max_retry = 20)
        ...
        m_field = pm::Mat(m_source.size(), PM_32SC3);
    ...
    inline pm::Size source_size() const {
        return m_source.size();
    }
    inline pm::Size target_size() const {
...
private:
    ...
    pm::Mat m_field;  // { y_target, x_target, distance_scaled }
...
public:
    RegularityGuidedPatchDistanceMetricV2(int patch_size, pm::Mat ijmap, double weight)
